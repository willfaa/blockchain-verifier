...
FILE_PATH="./testdata/medium.pdf"
COUNT=20

echo "=== Loadtest /issue dengan file medium (${FILE_PATH}), COUNT=${COUNT} ==="
...
for i in $(seq 1 $COUNT); do
  ...
  echo "[$(date)] #$i STATUS=$HTTP_STATUS TIME=${DURATION}ms" >> loadtest-issue-medium.log
